...
(function () {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                function getErrorMessageForKey(key) {
                    ...
                    if (attrs["type"] == "search") {
                        var message = attrs.$$element.parents('md-autocomplete').attr('msg-' + key[0].toUpperCase() + key.substring(1));
                        if (message == null)
                            return $interpolate(defaultErrors[key])(attrs);
                        return message;
                    }
    ...
    xtForm.directive('xtValidationInline', ["$templateCache", "$timeout", function ($templateCache, $timeout) {
        'use strict';
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                $timeout(function () {
                    ...
                    if (inputEl.length === 0) {
                        inputEl = angular.element(document.getElementsByName(inputId)[0]);
                        if (inputEl.length == 0)
...
angular.module('xtForm').run(['$templateCache', function ($templateCache) {
    $templateCache.put('xtForm/inline/validationInline.html',
        ...
        '    <span ng-message="{{error.key}}" ng-repeat="error in errors" data-key="{{error.key}}" class="ng-animate">{{error.message}}</span>\n' +
        '</div>');
}]);
